...
const run = async () => {
  ...
  await writeFile({ filePath: config.output.listPath, data: songs });

  _.each(_.keys(app.stats), service => {
    const { ignored, sorted } = app.stats[service];
    logger.info(`${service} parsed with ${ignored.length} ignored, ${sorted.length} identified songs`);
  });
